...
namespace TurboJpegWrapper
{
    static class TurboJpegImport
    {
        ...
        ///     </list>
        /// </summary>
        public static readonly Dictionary<TJSubsamplingOptions, Size> MCUSizes = new Dictionary
            <TJSubsamplingOptions, Size>
        {
        ...
        public static bool LibraryFound { get; private set; }

#if !NETSTANDARD1_3
        public static void Load()
        {
            Load(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location));
        }
#endif
        ...
        public static void Load(string directory)
        {
            ...
            if (Environment.OSVersion.Platform == PlatformID.Win32NT)
            {
                ...
                if (IntPtr.Size == 4)
                {
                    ...
                    resource = Properties.Resources.libturbojpeg_32;
                }
                else
                {
                    nativeLibraryDirectory = Path.Combine(directory, "x64");
                    resource = Properties.Resources.libturbojpeg_64;
                }

                Directory.CreateDirectory(nativeLibraryDirectory);

                var libraryFile = new FileInfo(Path.Combine(nativeLibraryDirectory, $"{UnmanagedLibrary}.dll"));
                ...
                if (!libraryFile.Exists || libraryFile.Length != resource.Length)
                {
                    if(libraryFile.Exists)
                        ...
                        {
                            libraryFile =
                                new FileInfo(Path.Combine(nativeLibraryDirectory, $"{Guid.NewGuid().ToString("N")}.dll"));
                        }
                ...
                IntPtr result = NativeMethods.LoadLibrary(libraryFile.FullName);
            ...
            else
            {
                throw new NotSupportedException(
                    "Quamotion.TurboJpegWrapper is supported on Windows (.NET FX, .NET Core), Linux (.NET Core) and OS X (.NET Core)");
            }
#else
        ...
        /// <returns>
        ///     A pointer to a list of fractional scaling factors, or <see cref="IntPtr.Zero" /> if an error is encountered
        ///     (see <see cref="tjGetErrorStr" />)
        /// </returns>
        [DllImport(UnmanagedLibrary, CallingConvention = CallingConvention.Cdecl)]
            ...
            int pitch, int height, int pixelFormat, int flags)
        {
            ...
            {
                case 4:
                    return tjDecompress2_x86(handle, jpegBuf, (uint) jpegSize, dstBuf, width, pitch, height, pixelFormat,
                        flags);
                case 8:
